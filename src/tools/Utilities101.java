...
public class Utilities101 {
	...
	public static float applyRatingBounds(float prediction){
		return boundValue(MIN_RATING, MAX_RATING, prediction);
	}
...
//			System.out.println("ConfigurationString: " + configurationString);
		...
		public static boolean isItemRelevant(int item, int user, DataModel dataModel) {
			...
			if (PREDICTION_RELEVANCE_MIN_RATING_FOR_RELEVANCE > 0) {
				// a variant
...
//				System.out.println("min relevance: " + Recommender101Impl.PREDICTION_RELEVANCE_MIN_RATING_FOR_RELEVANCE);
				if (rating >= PREDICTION_RELEVANCE_MIN_RATING_FOR_RELEVANCE) {
					return true;
				}
				else {
					return false;
			...
			if (PREDICTION_RELEVANCE_MIN_PERCENTAGE_ABOVE_AVERAGE > 0) {
				threshold = threshold + (threshold * (PREDICTION_RELEVANCE_MIN_PERCENTAGE_ABOVE_AVERAGE/(double) 100)); 
			}
			if (rating >= threshold) {
				result = true;
			}
...
//		System.out.println("map1: " + map1 + " \nmap 2: " + map2);
	...
	public static void createCosineSimilaritiesFile(String outFileName,
		...
		ZipEntry ze= new ZipEntry("cos-sim-vectors.zip");
		zos.putNextEntry(ze);
